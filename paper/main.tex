...
answered questions were based on the same proportion of correctly answered 
questions (see~\nameref{subsec:glmm}).
...
%those questions. In other words, our estimates of participants' knowledge
%for questions they answered correctly should (on average) be 
%
%In other words, since our knowledge estimates are computed 
%as a weighted proportion-correct score (where each held-in question's weight reflects
%its embedding-space distance from the held-out question; see
%Eqn.~\ref{eqn:prop}), if these weights are uninformative (e.g., randomly
%distributed), then we should expect that, on average, 
%
%% note that with correction, pcorrect of held in questions is now $\frac{n - 1}{q - 1}$ regardless of held-out question's label
%
%==================================
%
%In performing these analyses, our null hypothesis is that the knowledge
...
%%\textbf{NOTES FOR REWRITING RESULTS}
%%\begin{itemize}
%%    \item all questions
%%    \begin{itemize}
%%        \item quiz 1: odds ratio $(OR) = 8.126$, 95\%\ $\textnormal{CI} = [3.116,\ 20.123]$, likelihood-ratio test statistic $(\lambda_{LR}) = 17.002,\ p < 0.001$
%%        \item quiz 2: $OR = 14.902,\ 95\%\ \textnormal{CI} = [4.976,\ 39.807],\ \lambda_{LR} = 25.408,\ p < 0.001$
%%        \item quiz 3: $OR = 37.409,\ 95\%\ \textnormal{CI} = [10.425,\ 107.145],\ \lambda_{LR} = 40.948,\ p < 0.001$
%%        \item we can reliably predict success on individual held-out questions regardless of level of knowledge; predictive power grows as participants' knowledge space becomes better defined/structured 
%%    \end{itemize}
%%    \item within-lecture
%%    \begin{itemize}
%%        \item quiz 1, lecture 1: $OR = 15.934,\ 95\%\ \textnormal{CI} = [5.173,\ 38.005],\ \lambda_{LR} = 40.971,\ p < 0.001$
%%        \item quiz 1, lecture 2: $OR = 9.775,\ 95\%\ \textnormal{CI} = [2.93,\ 25.08],\ \lambda_{LR} = 13.924,\ p < 0.001$
%%        \item quiz 2, lecture 1: $OR = 35.126,\ 95\%\ \textnormal{CI} = [5.113,\ 123.868],\ \lambda_{LR} = 32.251,\ p < 0.001$
%%        \item quiz 2, lecture 2: $OR = 4.717,\ 95\%\ \textnormal{CI} = [2.021,\ 9.844],\ \lambda_{LR} = 16.788,\ p < 0.001$
%%        \item quiz 3, lecture 1: $OR = 2.485,\ 95\%\ \textnormal{CI} = [0.724,\ 8.366],\ \lambda_{LR} = 1.984,\ p = 0.170$
%%        \item quiz 3, lecture 2: $OR = 16.902,\ 95\%\ \textnormal{CI} = [3.353,\ 53.265],\ \lambda_{LR} = 23.233,\ p < 0.001$
%%        \item can successfully distinguish between knowledge for questions about different content within a lecture, but may be susceptible to ceiling effects (considering questions included in model, across-participants avg prop correct on quiz3, lecture-1 questions was highest of any set of questions we analyzed (mean: 0.795, std: 0.236)
%%    \end{itemize}
%%    \item across-lecture
%%    \begin{itemize}
%%        \item quiz 1, lecture 1: $OR = 1.896,\ 95\%\ \textnormal{CI} = [0.419,\ 9.088],\ \lambda_{LR} = 0.712,\ p = 0.404$
%%        \item quiz 1, lecture 2: $OR = 1.522,\ 95\%\ \textnormal{CI} = [0.332,\ 6.835],\ \lambda_{LR} = 0.286,\ p = 0.611$
%%        \item quiz 2, lecture 1: $OR = 3.49,\ 95\%\ \textnormal{CI} = [0.739,\ 12.849],\ \lambda_{LR} = 3.266,\ p = 0.083$
%%        \item quiz 2, lecture 2: $OR = 2.199,\ 95\%\ \textnormal{CI} = [0.711,\ 5.623],\ \lambda_{LR} = 2.304,\ p = 0.141$
%%        \item quiz 3, lecture 1: $OR = 11.294,\ 95\%\ \textnormal{CI} = [1.375,\ 47.744],\ \lambda_{LR} = 10.396,\ p < 0.001$
%%        \item quiz 3, lecture 2: $OR = 7.302,\ 95\%\ \textnormal{CI} = [1.077,\ 44.879],\ \lambda_{LR} = 4.708,\ p = 0.038$
%%        \item ability to generalize across content areas depends on participants having structured knowledge about both (A) the content used to estimate knowledge, and (B) the content for which knowledge is estimated (quiz 1: have neither; quiz 2: have one but not the other; quiz 3: have both)
%%    \end{itemize}
%%\end{itemize}
...
%0.001$). This relationship held and appeared to strengthen when we repeated this 
%analysis for Quiz~2 ($OR = 14.902,\ 95\%\ \textnormal{CI} = [4.976,\ 39.807],\ 
%\lambda_{LR} = 25.408,\ p < 0.001$), and again for Quiz~3 ($OR = 37.409,\ 95\%\ 
%\textnormal{CI} = [10.425,\ 107.145],\ \lambda_{LR} = 40.948,\ p < 0.001$).
%Taken together, these results suggest that our knowledge estimates reliably 
%predict participants' performance on individual questions when aggregated across
%all quiz content, and that their predictive power increases as participants gain
%additional knowledge about the content captured by the text embedding space.
...
25.08],\ \lambda_{LR} = 13.924,\ p = 0.001$). These within-lecture knowledge
estimates also predicted success on questions about both lectures when we 
computed them analogously for Quiz~2 (\textit{Four Fundamental Forces}: $OR = 
35.126,\ 95\%\ \textnormal{CI} = [5.113,\ 123.868],\ \lambda_{LR} = 32.251,\ 
p < 0.001$; \textit{Birth of Stars}: $OR = 4.717,\ 95\%\ \textnormal{CI} = 
[2.021,\ 9.844],\ \lambda_{LR} = 16.788,\ p < 0.001$). For Quiz~3, we found that
within-lecture knowledge estimates predicted participants' success on 
\textit{Birth of Stars}-related questions ($OR = 16.902,\ 95\%\ \textnormal{CI} 
= [3.353,\ 53.265],\ \lambda_{LR} = 23.233,\ p < 0.001$) but not on \textit{Four 
Fundamental Forces}-related questions ($OR = 2.485,\ 95\%\ \textnormal{CI} = 
[0.724,\ 8.366],\ \lambda_{LR} = 1.984,\ p = 0.170$). This may indicate that the 
within-lecture knowledge estimates are susceptible to ceiling effects in 
...
\textit{not} able to correctly answer. This broad homogeneity in participants' success on questions used to estimate their knowledge may have hurt our ability 
...
We computed a corrected $p$-value for our observed $\lambda_{LR}$ as $\frac{r+1}{n+1}$, where $r$ is the number of simulated model comparisons that yielded a $\lambda_{LR}$ greater than our observed value and $n$ is the number of simulations we ran (1,000).
