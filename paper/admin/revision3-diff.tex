...
%DIF DEL old.tex    Sun May 25 17:02:22 2025
%DIF ADD main.tex   Sun May 25 18:10:53 2025
\usepackage{amsmath}
%DIF 3d3
...
%DIF 18c16
%DIF < % supplemental tables
%DIF -------
% supplementary tables %DIF > 
%DIF -------
...
%DIF 22-23c20
%DIF < 
%DIF < % supplemental figures
%DIF -------
% supplementary figures %DIF > 
%DIF -------
...
%DIF -------
\newcommand{\individualKnowledgeMapsA}{5} %DIF > 
\newcommand{\individualKnowledgeMapsB}{6} %DIF > 
\newcommand{\individualKnowledgeMapsC}{7} %DIF > 
\newcommand{\individualLearningMapsA}{8} %DIF > 
\newcommand{\individualLearningMapsB}{9} %DIF > 
\newcommand{\ldaVsBERT}{10} %DIF > 
\newcommand{\jaccard}{11} %DIF > 
%DIF -------

%DIF 36c33-34
%DIF < \newcommand{\U}{{\fontfamily{serif}\selectfont\ensuremath{\mathrm{U}}}}
%DIF -------
% supplementary results %DIF > 
\newcommand{\suppResults}{\textit{Supplementary results}} %DIF > 
%DIF -------
...
%DIF -------
\renewcommand{\nameref}[1]{\textit{\oldnameref{#1}}} %DIF > 
%DIF -------
...
\DIFaddbegin \vspace{-0.5em}\DIFaddend \small{\textsuperscript{1}Department of Psychological and Brain
Sciences}\\
\small{Dartmouth College, Hanover, NH 03755, USA}\\
\DIFaddbegin \vspace{-0.5em}\DIFaddend \small{\textsuperscript{2}Akili Interactive Labs}\\
\small{Boston, MA 02110, USA}\\
\DIFaddbegin \vspace{-0.5em}\DIFaddend \small{\textsuperscript{*}Corresponding author:
...
representation space \DIFdelbegin \DIFdel{, }\DIFdelend where nearby coordinates reflect similar or related
...
watching two \DIFdelbegin \DIFdel{course videos from the Khan Academy platform}\DIFdelend \DIFaddbegin \DIFadd{Khan Academy course videos}\DIFaddend . We apply our
...
learners' knowledge changed after watching each video \DIFaddbegin \DIFadd{and predict their success
on individual quiz questions}\DIFaddend . Our findings show how a small set of quiz
...
their quiz questions correctly, or that they received a `B'. Here \DIFdelbegin \DIFdel{, }\DIFdelend we show that
...
sentences, paragraphs, chapters, etc.), \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{their }\DIFaddend temporal and grammatical
properties\DIFdelbegin \DIFdel{of the elements}\DIFdelend , and other high-level characteristics of
...
feature vectors to two conceptually related documents \DIFdelbegin \DIFdel{, }\DIFdelend \textit{even
...
Este86a, Este86b, GlucEtal02, AshbMadd05, HulbNorm15}. However\DIFaddbegin \DIFadd{, }\DIFaddend the objective of
...
learning. We \DIFaddbegin \DIFadd{show that these models recover meaningful relationships between
...
quiz questions. We }\DIFaddend also provide a demonstration of how our models can be used to
...
\begin{figure}[tp]
    ...
    knowledge. The specific questions \DIFdelbeginFL \DIFdelFL{reflected }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{appearing }\DIFaddendFL on each quiz, and the orders of
...
related content \DIFdelbegin \DIFdel{, }\DIFdelend so that we could test our approach's ability to distinguish
...
of ``topic proportions\DIFdelbegin \DIFdel{,}\DIFdelend '' describing the weighted blend of learned topics
...
correspondence could not \DIFdelbegin \DIFdel{solely arise }\DIFdelend \DIFaddbegin \DIFadd{arise solely }\DIFaddend from superficial text matching between
...
strongly correlated with the average topic weights from \DIFaddbegin \DIFadd{questions about }\DIFaddend Lecture~1
\DIFdelbegin \DIFdel{questions
}\DIFdelend ($r(13) = 0.809,~p < 0.001$, 95\% confidence interval (CI)~$= [0.633,~0.962]$),
and the average topic weights from Lecture~2 are strongly correlated with the
average topic weights from \DIFaddbegin \DIFadd{questions about }\DIFaddend Lecture~2 \DIFdelbegin \DIFdel{questions }\DIFdelend ($r(13) = 0.728,~p = 0.002$,
...
\begin{figure}[tp]
    ...
    weight variability}. The bar plots display the variance \DIFdelbeginFL \DIFdelFL{of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in }\DIFaddendFL each topic's
...
the topic's weights \DIFdelbegin \DIFdel{changed }\DIFdelend \DIFaddbegin \DIFadd{change }\DIFaddend in meaningful ways over time, \DIFdelbegin \DIFdel{the topic }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend would be a
poor indicator of any \textit{specific} conceptual content in the lecture. We
therefore also compared the variances in topic weights (\DIFdelbegin \DIFdel{across time or
}\DIFdelend \DIFaddbegin \DIFadd{over time and across
}\DIFaddend questions) between the lectures and questions. The variability in topic
expression \DIFdelbegin \DIFdel{(over time and across questions) }\DIFdelend was similar for the Lecture~1 video
and questions ($r(13) = 0.824,~p<0.001$, 95\% CI~$= [0.696,~0.973]$)\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, and for }\DIFaddend the
...
\DIFdelbegin \DIFdel{While an }\DIFdelend \DIFaddbegin \DIFadd{An }\DIFaddend individual lecture may be organized around a single broad theme at a
coarse scale, \DIFaddbegin \DIFadd{but }\DIFaddend at a finer scale, each moment of a lecture typically covers a
...
range of timepoints from their corresponding lectures, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{outside of which }\DIFaddend the
correlations fell off sharply \DIFdelbegin \DIFdel{outside of that range }\DIFdelend (Supp.~Figs.~\forcesCorrs,~\bosCorrs).
We also qualitatively examined the best-matching intervals for each question by
comparing the \DIFdelbegin \DIFdel{question' s }\DIFdelend \DIFaddbegin \DIFadd{questions' }\DIFaddend text to the transcribed text from the most-correlated parts of the
...
proportion of \DIFdelbegin \DIFdel{correctly answered questions }\DIFdelend \DIFaddbegin \DIFadd{questions the student answered correctly}\DIFaddend . But if two students receive
identical scores on such an \DIFdelbegin \DIFdel{exam}\DIFdelend \DIFaddbegin \DIFadd{assessment}\DIFaddend , might our modeling framework help us to gain more
...
responses to a small set of multiple-choice questions to estimate how much \DIFdelbegin \DIFdel{the
}\DIFdelend \DIFaddbegin \DIFadd{that
}\DIFaddend participant ``knows'' about the concept reflected by any arbitrary coordinate
...
\DIFdelbegin \DIFdel{the }\DIFdelend participants knew about any moment's content, from either of the two
...
\begin{figure}[tp]
    ...
    lecture. \DIFdelbeginFL \DIFdelFL{All panels: error }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{All panels.}} \DIFaddFL{Error }\DIFaddendFL ribbons and error bars denote 95\% confidence
...
knowledge about the \textit{Birth of Stars} lecture content on \DIFdelbegin \DIFdel{Quizzes}\DIFdelend \DIFaddbegin \DIFadd{Quiz}\DIFaddend ~1 versus
2 ($t(49) = 1.013,~p = 0.316$), but \DIFdelbegin \DIFdel{the }\DIFdelend estimated knowledge was substantially
...
Equation~\ref{eqn:prop} to estimate their knowledge at \DIFdelbegin \DIFdel{the given }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend question's
\DIFdelbegin \DIFdel{embedding space }\DIFdelend \DIFaddbegin \DIFadd{embedding-space }\DIFaddend coordinate based on other questions that participant answered
...
mixed model (GLMM) with a logistic link function to explain the \DIFdelbegin \DIFdel{likelihood }\DIFdelend \DIFaddbegin \DIFadd{probability }\DIFaddend of
correctly answering a question as a function of estimated knowledge \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend its
embedding coordinate, while accounting for \DIFdelbegin \DIFdel{random variation among }\DIFdelend \DIFaddbegin \DIFadd{varied effects of individual
}\DIFaddend participants and questions (see \nameref{subsec:glmm}). To assess the predictive
value of the knowledge estimates, we compared each GLMM to an analogous (i.e.,
nested) ``null'' model that \DIFdelbegin \DIFdel{did not consider estimated knowledge }\DIFdelend \DIFaddbegin \DIFadd{assumed these estimates carried no predictive information }\DIFaddend using parametric bootstrap likelihood-ratio tests.
...
\begin{figure}[tp]
    ...
    \DIFdelbeginFL \DIFdelFL{likelihood }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{probability }\DIFaddendFL of correctly answering a quiz question as a function of
...
at each \DIFaddbegin \DIFadd{held-out }\DIFaddend question's embedding coordinate using \textit{all} other questions
...
about one lecture using only \DIFaddbegin \DIFadd{the }\DIFaddend questions (from the same participant and quiz)
...
\DIFdelbegin \DIFdel{held }\DIFdelend \DIFaddbegin \DIFadd{could extend }\DIFaddend across the content areas of the two lectures. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In performing these analyses, our null hypothesis is that the knowledge
estimates we compute based on the quiz questions' embedding coordinates do
}\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{provide useful information about participants' abilities to answer
...
carried out these same analyses using a
simple proportion-correct measure in
lieu of our knowledge estimates . Suppose a participant correctly }\DIFdelend \DIFaddbegin \DIFadd{When estimating
...
of correctly }\DIFaddend answered \DIFdelbegin \DIFdel{$n$
out of $q$ questions on a given quiz. If we hold out a single
}\textit{\DIFdel{correctly}} %DIFAUXCMD
\DIFdel{answered question, the proportion of remaining questions
answered correctly would be $\frac{n - 1}{q - 1}$. Whereas if we hold out a
single }\textit{\DIFdel{incorrectly}} %DIFAUXCMD
\DIFdel{answered question, the proportion of remaining
questions answered correctly would be $\frac{n}{q - 1}$. In this way, the
proportion of correctly answered remaining questions is always }\textit{\DIFdel{lower}}
%DIFAUXCMD
\DIFdel{when the held-out question was answered correctly than when it was answered
...
the spatial relationships among the quiz questions' embeddings }\textit{\DIFdel{are}}
%DIFAUXCMD
\DIFdel{predictive of participants' knowledge about the questions' content, then we
would expect }\textit{\DIFdel{higher}} %DIFAUXCMD
\DIFdel{estimated knowledge for held-out correctly (versus
incorrectly) answered questions }\DIFdelend \DIFaddbegin \DIFadd{questions (see~}\nameref{subsec:glmm}\DIFadd{)}\DIFaddend .

\DIFdelbegin \DIFdel{Before presenting our results, it is worth considering three possible
explanations of why a participant might answer a given question correctly or
incorrectly. One possibility is that the participant simply }\textit{\DIFdel{guessed}}
%DIFAUXCMD
\DIFdel{the answer. A second is that they selected the incorrect answer by mistake,
...
participant's response reflects their }\textit{\DIFdel{actual}} %DIFAUXCMD
\DIFdel{knowledge about the
question's content. In this case, we }\textit{\DIFdel{might}} %DIFAUXCMD
\DIFdel{expect to seea positive
...
reflected (at least partially)by the embedding space.
...
%DIFDELCMD < %%%
\DIFdelend When we fit a GLMM to estimates of participants' knowledge for each Quiz~1
question based on all other Quiz~1 questions, we \DIFdelbegin \DIFdel{observed an outcome consistent
with our null hypothesis: }\DIFdelend \DIFaddbegin \DIFadd{found that }\DIFaddend higher estimated
knowledge \DIFdelbegin \DIFdel{at the embedding
coordinate of a held-out question was associated with a lower }\DIFdelend \DIFaddbegin \DIFadd{for a given question predicted a greater }\DIFaddend likelihood of answering \DIFdelbegin \DIFdel{the question }\DIFdelend \DIFaddbegin \DIFadd{it
}\DIFaddend correctly (odds ratio \DIFdelbegin \DIFdel{$(OR) = 0.136$, likelihood-ratio
test statistic $(\lambda_{LR}) = 19.749$, }\DIFdelend \DIFaddbegin \DIFadd{$(OR) = 8.126$, }\DIFaddend 95\%\ \DIFdelbegin \DIFdel{$\textnormal{CI} = [14.352,\
26.545],\ p < 0.001$}\DIFdelend \DIFaddbegin \DIFadd{$\textnormal{CI} = [3.116,\
20.123]$, likelihood-ratio test statistic $(\lambda_{LR}) = 17.002,\ p <
0.001$}\DIFaddend ). This \DIFdelbegin \DIFdel{outcome suggests that our knowledge estimates do
}\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{provide useful information about participants' Quiz~1 performance
...
questions (prior to having watched either lecture). When we }\DIFdelend \DIFaddbegin \DIFadd{relationship held when we }\DIFaddend repeated this analysis for \DIFdelbegin \DIFdel{Quizzes}\DIFdelend \DIFaddbegin \DIFadd{Quiz}\DIFaddend ~2 \DIFdelbegin \DIFdel{and}\DIFdelend \DIFaddbegin \DIFadd{($OR
= 14.902,\ 95\%\ \textnormal{CI} = [4.976,\ 39.807],\ \lambda_{LR} = 25.408,\
p < 0.001$) and again for Quiz}\DIFaddend ~3 \DIFdelbegin \DIFdel{, we found that higher estimated knowledge
for a given question predicted a }\textit{\DIFdel{greater}} %DIFAUXCMD
\DIFdel{likelihood of answering it correctly
(Quiz~2: $OR = 2.905,\ \lambda_{LR} = 17.333,\ 95\%\ \textnormal{CI} =
[14.966,\ 29.309],\ p = 0.001$; Quiz~3: $OR = 3.238,\ \lambda_{LR} = 6.882,\
95\%\ \textnormal{CI} = [6.228,\ 8.184],\ p = 0.016$}\DIFdelend \DIFaddbegin \DIFadd{($OR = 37.409,\ 95\%\ \textnormal{CI} =
[10.425,\ 107.145],\ \lambda_{LR} = 40.948,\ p < 0.001$}\DIFaddend ). Taken together, these
results suggest that our knowledge estimates \DIFaddbegin \DIFadd{can }\DIFaddend reliably predict participants'
performance on individual \DIFdelbegin \DIFdel{held-out quiz questions , but only after participants
have received at least some training}\DIFdelend \DIFaddbegin \DIFadd{questions when they incorporate information from all
(other) quiz content}\DIFaddend .

We observed a similar \DIFdelbegin \DIFdel{pattern }\DIFdelend \DIFaddbegin \DIFadd{set }\DIFaddend of results when \DIFdelbegin \DIFdel{used this approach to estimate
}\DIFdelend \DIFaddbegin \DIFadd{we restricted our estimates of
}\DIFaddend participants' knowledge \DIFdelbegin \DIFdel{about held-out questions from one lecture using }\DIFdelend \DIFaddbegin \DIFadd{to consider only
}\DIFaddend their performance on other questions \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{about }\DIFaddend the \textit{same} lecture.
Specifically, for Quiz~1\DIFdelbegin \DIFdel{questions (i.e., prior to watching either), participants' estimated
knowledge for the embedding coordinates of held-out }\DIFdelend \DIFaddbegin \DIFadd{, participants' knowledge of }\DIFaddend \textit{Four Fundamental
Forces}-related questions\DIFdelbegin \DIFdel{estimated using }\DIFdelend \DIFaddbegin \DIFadd{, estimated from their performance on }\DIFaddend other
\textit{Four Fundamental Forces}-related questions\DIFdelbegin \DIFdel{did not reliably predict whether those questions were
answered correctly ($OR = 1.891,\ \lambda_{LR} = 2.293,\ 95\%\ \textnormal{CI}
= [2.091,\ 2.622],\ p = 0.138$}\DIFdelend \DIFaddbegin \DIFadd{, was predictive of their
ability to answer those questions correctly ($OR = 15.934,\ 95\%\
\textnormal{CI} = [5.173,\ 38.005],\ \lambda_{LR} = 40.971,\ p = 0.001$}\DIFaddend ). The
same was true of \DIFdelbegin \DIFdel{knowledge estimates for held-out }\DIFdelend \DIFaddbegin \DIFadd{participants' estimated knowledge for }\DIFaddend \textit{Birth of
Stars}-related questions based on \DIFaddbegin \DIFadd{their performance on }\DIFaddend other \textit{Birth of
Stars}-related questions (\DIFdelbegin \DIFdel{$OR = 0.722,\ \lambda_{LR} = 5.115,\ 95\%\
...
analyses using questions from Quiz~}\DIFdelend \DIFaddbegin \DIFadd{$OR = 9.775,\ 95\%\ \textnormal{CI} = [2.93,\
25.08],\ \lambda_{LR} = 13.924,\ p = 0.001$). These results also held for
participants' Quiz~}\DIFaddend 2 \DIFdelbegin \DIFdel{(which participants took immediately after
viewing }\DIFdelend \DIFaddbegin \DIFadd{responses (}\DIFaddend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{but prior to viewing }\DIFdelend \DIFaddbegin \DIFadd{: $OR =
35.126,\ 95\%\ \textnormal{CI} = [5.113,\ 123.868],\ \lambda_{LR} = 32.251,\
p < 0.001$; }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{), we found that they now reliably predicted success on }\textit{\DIFdel{Four
Fundamental Forces}}%DIFAUXCMD
\DIFdel{-related questions ($OR = 9.023,\ \lambda_{LR} = 18.707,\
95\%\ \textnormal{CI} = [10.877,\ 22.222],\ p < 0.001$) but not on
}\DIFdelend \DIFaddbegin \DIFadd{: $OR = 4.717,\ 95\%\ \textnormal{CI} =
[2.021,\ 9.844],\ \lambda_{LR} = 16.788,\ p < 0.001$) and partially for 
their Quiz~3 responses (}\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{-related questions ($OR = 0.306,\ \lambda_{LR} = 5.115,\
95\%\ \textnormal{CI} = [4.624,\ 5.655],\ p = 0.054$). Here, we speculate that participants might have been guessing about the }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{content
(e.g., prior to having watched it), whereas they might have been drawing on
some structured knowledge about the }\DIFdelend \DIFaddbegin \DIFadd{: $OR = 16.902,\ 95\%\ 
\textnormal{CI} = [3.353,\ 53.265],\ \lambda_{LR} = 23.233,\ p < 0.001$; 
}\DIFaddend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{content
(e.g., from having just watched it). When we applied this approach to }\DIFdelend \DIFaddbegin \DIFadd{: $OR = 2.485,\ 95\%\ \textnormal{CI} =
...
participants' quiz performance. On }\DIFaddend Quiz~3\DIFdelbegin \DIFdel{responses (given immediately after viewing }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{), we found
that within-lecture knowledge estimates for }\DIFdelend \DIFaddbegin \DIFadd{, after viewing both lectures, no
participant answered more than three }\DIFaddend \textit{\DIFdelbegin \DIFdel{Birth of Stars}\DIFdelend \DIFaddbegin \DIFadd{Four Fundamental Forces}\DIFaddend }-related
questions \DIFdelbegin \DIFdel{could now reliably predict success on those questions ($OR = 5.467,\
\lambda_{LR} = 10.670,\ 95\%\ \textnormal{CI} = [7.998, 12.532],\ p = 0.005$) .
However, 
}\DIFdelend \DIFaddbegin \DIFadd{incorrectly, and all but five participants (out of 50) answered two or
...
participants, our }\DIFaddend within-lecture \DIFdelbegin \DIFdel{knowledge estimates }\DIFdelend \DIFaddbegin \DIFadd{estimates of their knowledge }\DIFaddend for \textit{Four 
Fundamental Forces}\DIFdelbegin \DIFdel{questions answered on Quiz~3 were no longer directly related to 
}\DIFdelend \DIFaddbegin \DIFadd{-related questions that they answered incorrectly leveraged 
information from at most a single other question they were }\textit{\DIFadd{not}} \DIFadd{able to 
...
lecture content these participants did }\textit{\DIFadd{not}} \DIFadd{know about, and successfully 
distinguish them from the far more numerous aspects of }\DIFaddend the \DIFdelbegin \DIFdel{likelihood of successfully answering them and instead exhibited the inverse
...
that this may reflect participants forgetting some of the 
}\textit{\DIFdel{Four
Fundamental Forces}} %DIFAUXCMD
\DIFdel{content (e.g., perhaps in favor of prioritizing encoding
the just-watched }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{content in preparation for the third
...
held-out questions about}\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{were answered
...
participants answered incorrectly. }\DIFdelend \DIFaddbegin \DIFadd{lecture content they 
now }\textit{\DIFadd{did}} \DIFadd{know about. }\DIFaddend Taken together, these within-lecture results suggest that our 
\DIFdelbegin \DIFdel{approach can }\DIFdelend \DIFaddbegin \DIFadd{knowledge estimates can reliably }\DIFaddend distinguish between questions about different
content covered by a single lecture\DIFdelbegin \DIFdel{when participantshave sufficiently
structured knowledge about its contents, though this specificity may decrease
with time since the relevant material was learned}\DIFdelend \DIFaddbegin \DIFadd{, provided there is sufficient diversity in 
participants' quiz responses to extract meaningful information about both what 
they know and what they do not know}\DIFaddend .

Finally, we \DIFdelbegin \DIFdel{used this approach to estimate }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend participants' knowledge \DIFdelbegin \DIFdel{about
held-out questions from one lecture using }\DIFdelend \DIFaddbegin \DIFadd{for each question about each
lecture using only }\DIFaddend their performance on questions \DIFdelbegin \DIFdel{from
the }\DIFdelend \DIFaddbegin \DIFadd{(from the same quiz) about the
}\DIFaddend \textit{other} lecture. \DIFdelbegin \DIFdel{Here we again observed a similar pattern of results, though with some notable differences.
On Quiz~}\DIFdelend \DIFaddbegin \DIFadd{This is an especially stringent test of our approach.
...
\DIFadd{We found that, before viewing either lecture (i.e., on Quiz~}\DIFaddend 1\DIFdelbegin \DIFdel{, we found that participants' abilities to correctly answer questions about }\DIFdelend \DIFaddbegin \DIFadd{), participants'
abilities to answer }\DIFaddend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{could be predicted from their responses to questions about }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{($OR = 1.896,\ \lambda_{LR} = 7.205,\ 95\%\
\textnormal{CI} = [6.224, 7.524],\ p = 0.038$) and similarly, that their
ability to correctly answer }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdelend -related \DIFdelbegin \DIFdel{questions could be
predicted from their responses to }\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{-related
...
questions and within-lecture predictions, we were surprised to find that the knowledge estimates could reliably (if weakly) predict participants'
performance across content from
different lectures . It is possible that this
...
participantshad about the content prior to watching either lecture . When we
repeated this analysis using questions from Quiz~2, we found participants'
responses to }\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{-related }\DIFdelend questions \DIFdelbegin \DIFdel{did
}\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{reliably predict their success on }\DIFdelend \DIFaddbegin \DIFadd{could
not be predicted from their responses to }\DIFaddend \textit{Birth of Stars}-related
questions (\DIFdelbegin \DIFdel{$OR = 1.865,\ \lambda_{LR} = 3.205,\ 95\%\ \textnormal{CI} = [3.027,
3.600],\ p = 0.124$}\DIFdelend \DIFaddbegin \DIFadd{$OR = 1.896,\ 95\%\ \textnormal{CI} = [0.419,\ 9.088],\ \lambda_{LR}
= 0.712,\ p = 0.404$}\DIFaddend ), nor \DIFdelbegin \DIFdel{did their responses to }\DIFdelend \DIFaddbegin \DIFadd{could their abilities to answer }\DIFaddend \textit{Birth of
Stars}-related questions \DIFdelbegin \DIFdel{reliably predict their success on }\DIFdelend \DIFaddbegin \DIFadd{be predicted from their responses to }\DIFaddend \textit{Four
Fundamental Forces}-related questions (\DIFdelbegin \DIFdel{$OR = 3.490,\ \lambda_{LR} = 3.266,\
95\%\ \textnormal{CI} = [3.033, 3.866],\ p = 0.093$). These ``prediction
failures'' appear to come from the fact that
any signal derived from
participants' knowledge about the content of the }\textit{\DIFdel{Birth of Stars}}
%DIFAUXCMD
\DIFdel{lecture (prior to watching it) is overwhelmed by the much more dramatic increase in
their knowledge about the content of the }\textit{\DIFdel{Four Fundamental Forces}}
%DIFAUXCMD
\DIFdel{(which they watched just prior to taking Quiz~2). This is reflected in their
Quiz~2 performance for questions about each lecture (mean proportion correct
for }\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{-related }\DIFdelend \DIFaddbegin \DIFadd{$OR = 1.522,\ 95\%\ \textnormal{CI} =
...
predicted given their responses to }\DIFaddend questions \DIFaddbegin \DIFadd{about the other lecture after
viewing }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{but before viewing }\textit{\DIFadd{Birth of
Stars}} \DIFadd{(i.e., }\DIFaddend on Quiz~2\DIFdelbegin \DIFdel{: $0.77$; mean
proportion correct for }\DIFdelend \DIFaddbegin \DIFadd{; }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions given
}\DIFaddend \textit{Birth of Stars} \DIFdelbegin \DIFdel{-related questions on Quiz~2: $0.36$). When we carried out these across-lecture knowledge predictions using
questions from }\DIFdelend \DIFaddbegin \DIFadd{questions: $OR = 3.49,\ 95\%\ \textnormal{CI} =
[0.739,\ 12.849],\ \lambda_{LR} = 3.266,\ p = 0.083$; }\textit{\DIFadd{Birth of Stars}}
\DIFadd{questions given }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions: $OR = 2.199,\ 95\%\
\textnormal{CI} = [0.711,\ 5.623],\ \lambda_{LR} = 2.304,\ p = 0.141$). Only
after viewing }\textit{\DIFadd{both}} \DIFadd{lectures (i.e., on }\DIFaddend Quiz~3\DIFdelbegin \DIFdel{(when participants had now viewed }\textit{\DIFdel{both}}
%DIFAUXCMD
\DIFdel{lectures) , we could again reliably predict success on }\DIFdelend \DIFaddbegin \DIFadd{) did these across-lecture
knowledge estimates reliably predict participants' success on individual quiz
questions (}\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions given }\textit{\DIFadd{Birth of
Stars}} \DIFaddend questions\DIFdelbegin \DIFdel{about both
}\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{($OR = 11.294),\ \lambda_{LR} = 11.055,\ 95\%\
\textnormal{CI} = [9.126, 18.476],\ p = 0.003$) and }\DIFdelend \DIFaddbegin \DIFadd{: $OR = 11.294,\ 95\%\ \textnormal{CI} = [1.375,\ 47.744],\
\lambda_{LR} = 10.396,\ p < 0.001$; }\DIFaddend \textit{Birth of Stars} \DIFdelbegin \DIFdel{($OR = 7.302,\ \lambda_{LR} = 7.068,\ 95\%\ \textnormal{CI} = [6.490, 8.584],\
p = 0.016$) using responses to questionsabout the other lecture's content. Across all three versions of these analyses, our }\DIFdelend \DIFaddbegin \DIFadd{questions given
}\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions: $OR = 7.302,\ 95\%\ \textnormal{CI}
= [1.077,\ 44.879],\ \lambda_{LR} = 4.708,\ p = 0.038$). Taken together, these
}\DIFaddend results suggest that \DIFdelbegin \DIFdel{(by and
large) our knowledge estimates can reliably predict participants' abilities to answer individual quiz questions , distinguish between questionsabout similar
content , and generalize across }\DIFdelend \DIFaddbegin \DIFadd{our ability to form estimates solely across different
...
have recently received some training on both }\DIFaddend content areas, \DIFdelbegin \DIFdel{provided that participants' quiz
responses reflect a minimum level of ``real'' knowledge about both content on which these predictions are based and that for which they are made}\DIFdelend \DIFaddbegin \DIFadd{the knowledge 
estimates appear to be informative even across content areas.
}

\DIFadd{We speculate that these ``Across-lecture'' results might relate to some of our
earlier work on the nature of semantic representations~\mbox{%DIFAUXCMD
\citep{MannKaha12}}\hskip0pt%DIFAUXCMD
. In 
...
differed from the embeddings used to }\textit{\DIFadd{characterize}} \DIFadd{their behaviors. We 
...
}\textit{\DIFadd{related}} \DIFadd{to participants' initial internal representations; otherwise we 
...
derived from Quiz~3 do}\DIFaddend .
...
(Fig.~\ref{fig:predictions}) suggests that \DIFdelbegin \DIFdel{spatial }\DIFdelend \DIFaddbegin \DIFadd{geometric }\DIFaddend relationships within this
...
question \textit{\DIFdelbegin \DIFdel{in}\DIFdelend \DIFaddbegin \DIFadd{incorrectly}\DIFaddend }\DIFdelbegin \DIFdel{correctly}\DIFdelend . Again, as we move farther away from $x$ in the
embedding space, how does the likelihood that the participant does \textit{not}
know about \DIFdelbegin \DIFdel{a coordinate's content }\DIFdelend \DIFaddbegin \DIFadd{the content at a given coordinate }\DIFaddend change with distance? We reasoned that,
...
the question embedded \textit{at} $x$. \DIFdelbegin \DIFdel{Whereas at another extreme, }\DIFdelend \DIFaddbegin \DIFadd{But }\DIFaddend once we
...
beyond the maximum distance at which \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend participant's ability to answer the
question at $x$ is informative of their ability to answer a second question at
location $y$, \DIFdelbegin \DIFdel{then }\DIFdelend guessing the outcome at $y$ based on \DIFaddbegin \DIFadd{the outcome at }\DIFaddend $x$ should be no more
successful than guessing based on a measure that does not consider
\DIFdelbegin \DIFdel{embedding
space }\DIFdelend \DIFaddbegin \DIFadd{embedding-space }\DIFaddend distance.
...
With these ideas in mind, we asked: conditioned on \DIFdelbegin \DIFdel{answering a }\DIFdelend \DIFaddbegin \DIFadd{a participant's ability to
answer a given }\DIFaddend question correctly, what proportion of all questions \DIFdelbegin \DIFdel{(}\DIFdelend within some
radius \DIFdelbegin \DIFdel{, }\DIFdelend $r$ \DIFdelbegin \DIFdel{, of that
question's embedding coordinate ) were answered }\DIFdelend \DIFaddbegin \DIFadd{of its embedding coordinate were they able to answer }\DIFaddend correctly? We
plotted this proportion as a function of $r$ \DIFdelbegin \DIFdel{. Similarly, we could ask, conditioned on
answering a question incorrectly, how the proportion of correct responses
changed with $r$}\DIFdelend \DIFaddbegin \DIFadd{for questions that participants
answered correctly, and for questions they answered incorrectly}\DIFaddend .
As shown in Figure~\ref{fig:smoothness}, we found that quiz
performance falls off smoothly with distance, and the ``rate'' \DIFdelbegin \DIFdel{of the falloff
}\DIFdelend \DIFaddbegin \DIFadd{at which it falls off
}\DIFaddend does not appear to \DIFdelbegin \DIFdel{change across the different }\DIFdelend \DIFaddbegin \DIFadd{differ across }\DIFaddend quizzes, as measured by the
distance at which performance becomes statistically indistinguishable from a
simple \DIFdelbegin \DIFdel{proportion correct }\DIFdelend \DIFaddbegin \DIFadd{proportion-correct }\DIFaddend score (see~\nameref{subsec:smoothness}). This
suggests that, at least within the region of text embedding space \DIFdelbegin \DIFdel{covered }\DIFdelend \DIFaddbegin \DIFadd{spanned }\DIFaddend by
the questions our \DIFaddbegin \DIFadd{study's }\DIFaddend participants answered (and as characterized using our topic
...
quizzes \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend regions of the embedding space.

Knowledge estimates need not be limited to the \DIFdelbegin \DIFdel{content of the lectures }\DIFdelend \DIFaddbegin \DIFadd{contents of these particular
lectures and quizzes}\DIFaddend . As illustrated in Figure~\ref{fig:knowledge-maps}, our
...
beyond the lectures participants watched \DIFaddbegin \DIFadd{and the questions they answered}\DIFaddend , we
...
not precisely the same as) that contained in the two lectures \DIFdelbegin \DIFdel{. We note that we used these 2D maps solely for
visualization; all relevant comparisons, distance computations, and statistical
tests we report above were carried out in the
original 15-dimensional space,
using the 15-topic }\DIFdelend \DIFaddbegin \DIFadd{used to train the
}\DIFaddend model. Aside from increasing the number of topics from 15 to 100, all other procedures
...
\begin{figure}[tp]
    ...
    displays a 2D projection of \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{participants' }\DIFaddendFL estimated knowledge about the content
    reflected by \textit{all} regions of topic space (see
    \nameref{subsec:knowledge-maps}). The topic trajectories of the two
    lectures are indicated by dotted lines (blue: Lecture\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL 1; green: Lecture\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL 2),
    and the coordinates of each question are indicated by dots (light blue:
    Lecture\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL 1-related; light green: Lecture\DIFaddbeginFL \DIFaddFL{~}\DIFaddendFL 2-related; purple: general physics
    ...
    starred location, and their colors indicate their relative weights in \DIFaddbeginFL \DIFaddFL{the
    }\DIFaddendFL \textit{Four Fundamental Forces} (blue) versus \textit{Birth of Stars}
...
\DIFdelbegin \DIFdel{video }\DIFdelend \DIFaddbegin \DIFadd{the lectures }\DIFaddend and each quiz question) onto a shared 2-dimensional plane (see
...
projections of the \DIFdelbegin \DIFdel{videos }\DIFdelend \DIFaddbegin \DIFadd{lectures }\DIFaddend and questions. We \DIFaddbegin \DIFadd{then }\DIFaddend used
...
from a given quiz's responses provides a visualization of \DIFdelbegin \DIFdel{how ``}\DIFdelend \DIFaddbegin \DIFadd{``how }\DIFaddend much''
...
\DIFaddbegin \DIFadd{We note that we used these 2D maps solely for visualization; all relevant
...
yields a \textit{learning map} that describes the \textit{change} in \DIFdelbegin \DIFdel{knowledge
estimates }\DIFdelend \DIFaddbegin \DIFadd{estimated
knowledge }\DIFaddend from one quiz to the next. These learning maps highlight that the
...
two lectures' embeddings (pink). As shown in the word clouds in \DIFdelbegin \DIFdel{the panel}\DIFdelend \DIFaddbegin \DIFadd{Panel C}\DIFaddend , the
...
topics expressed in \textit{that} lecture. \DIFdelbegin \DIFdel{And the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend top-weighted words at the
...
knowledge participants have about each \DIFdelbegin \DIFdel{video}\DIFdelend \DIFaddbegin \DIFadd{lecture}\DIFaddend 's time-varying content, and
capture temporally specific increases in knowledge after viewing each lecture
(Fig.~\ref{fig:knowledge-timeseries}). We \DIFdelbegin \DIFdel{also }\DIFdelend \DIFaddbegin \DIFadd{then }\DIFaddend show that these knowledge
estimates can generalize to held-out questions \DIFaddbegin \DIFadd{and predict participants'
abilities to answer them correctly }\DIFaddend (Fig.~\ref{fig:predictions}). Finally, we use
...
\DIFdelbegin \DIFdel{We view our work as making }\DIFdelend \DIFaddbegin \DIFadd{Our work makes }\DIFaddend several contributions to the study of how people acquire
...
\DIFdelbegin \DIFdel{multiple choice }\DIFdelend \DIFaddbegin \DIFadd{multiple-choice }\DIFaddend quiz questions. Our experimental finding that we can use these 
maps to predict \DIFdelbegin \DIFdel{responses to }\DIFdelend \DIFaddbegin \DIFadd{success on }\DIFaddend held-out questions has several psychological 
...
coordinates. How participants' knowledge \DIFdelbegin \DIFdel{falls off }\DIFdelend \DIFaddbegin \DIFadd{``falls off'' }\DIFaddend with spatial distance is
...
related concepts, and \DIFdelbegin \DIFdel{we also show how estimated knowledge falls off with
distance }\DIFdelend \DIFaddbegin \DIFadd{how far this implication extends }\DIFaddend in text embedding space.

In our study, we characterize the ``coordinates'' of participants' knowledge
using a relatively simple ``\DIFdelbegin \DIFdel{bag of words}\DIFdelend \DIFaddbegin \DIFadd{bag-of-words}\DIFaddend '' text embedding model~\citep[LDA;
][]{BleiEtal03}. More sophisticated text embedding models, such as
transformer-based models~\citep{ViswEtal17, DevlEtal18, ChatGPT, TouvEtal23}\DIFdelbegin \DIFdel{can learn }\DIFdelend \DIFaddbegin \DIFadd{,
can leverage additional textual information such as }\DIFaddend complex grammatical and 
...
\DIFaddbegin \DIFadd{(see \suppResults)}\DIFaddend . We suspect that this reflects a broader challenge in 
constructing models that are \DIFaddbegin \DIFadd{both }\DIFaddend high-resolution within a given domain (e.g., 
the domain of physics lectures and questions) \textit{and} sufficiently broad 
\DIFdelbegin \DIFdel{so }\DIFdelend as to enable them to cover a wide range of domains. \DIFdelbegin \DIFdel{For example, 
we
found that the embeddings derived even from much largerand more modern models like BERT~\mbox{%DIFAUXCMD
...
LDA embeddings for }\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{questions (blue dots)
appear closer to the }\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{lecture trajectory (blue
line), whereas the LDA embeddings for }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{questions (green
dots) appear closer to the }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{lecture trajectory (green
...
similar. This tells us that LDA is capturing some differences in contentbetween the questions, whereas BERT is not. The time series plots of individual
...
contentof the questions versus specific moments of each lecture. 
Finally, we
also compared the pairwise correlations between embeddings of questions within
versus across content areas (i.e., content covered by the individual lectures }\DIFdelend \DIFaddbegin \DIFadd{Essentially, 
...
content}\DIFaddend , \DIFdelbegin \DIFdel{lecture-specific questions, and by the ``general physics knowledge''
...
questions about the }\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{material are highly
correlated with the embeddings of the }\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{lecture,
but not with the embeddings of }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{, questions about
}\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{, or general physics knowledge questions. We see a
similar pattern with the LDA embeddings of the }\textit{\DIFdel{Birth of Stars}}
%DIFAUXCMD
\DIFdel{questions (Fig.~\ref{fig:topics}, Supp.~Fig.~\topicWeights). In contrast, the
BERTembeddings are all highly correlated with each other (Supp.
Fig.~\ldaVsBERT). Taken together, these comparisons illustrate how LDA (trained
on the specific content in question) }\DIFdelend \DIFaddbegin \DIFadd{sacrificing a degree of specificity in their capacities to distinguish 
...
study) to a larger transformer-based model (BERT), we found that our LDA model }\DIFaddend provides 
both coverage of the requisite material and specificity at the level of 
\DIFdelbegin \DIFdel{the content covered by individual questions. BERT, on the other hand, essentially assigns }\DIFdelend \DIFaddbegin \DIFadd{individual questions, while BERT essentially relegates the contents of }\DIFaddend both 
lectures and all \DIFdelbegin \DIFdel{of the }\DIFdelend \DIFaddbegin \DIFadd{quiz }\DIFaddend questions (which are all broadly about ``physics'') \DIFdelbegin \DIFdel{into }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend a 
...
\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(Supp.~Fig.~\ldaVsBERT). }\DIFaddend We note that these are not criticisms of BERT\DIFdelbegin \DIFdel{(or }\DIFdelend \DIFaddbegin \DIFadd{, nor of 
}\DIFaddend other large language models trained on large and diverse corpora\DIFdelbegin \DIFdel{)}\DIFdelend . Rather, our 
point is that \DIFdelbegin \DIFdel{simple fine-tuned }\DIFdelend \DIFaddbegin \DIFadd{simpler }\DIFaddend models trained on \DIFdelbegin \DIFdel{a }\DIFdelend relatively small but specialized 
\DIFdelbegin \DIFdel{corpus }\DIFdelend \DIFaddbegin \DIFadd{corpora }\DIFaddend can outperform much more \DIFdelbegin \DIFdel{complicated }\DIFdelend \DIFaddbegin \DIFadd{complex }\DIFaddend models trained on much larger corpora 
\DIFdelbegin \DIFdel{, }\DIFdelend when we are specifically interested in capturing subtle conceptual differences 
at the level of a single\DIFaddbegin \DIFadd{, narrowly focused }\DIFaddend course lecture or \DIFdelbegin \DIFdel{question. Of course }\DIFdelend \DIFaddbegin \DIFadd{quiz question. On the other hand, }\DIFaddend if 
our goal had been to \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{choose }\DIFaddend a model that generalized to many different content 
areas \DIFaddbegin \DIFadd{simultaneously}\DIFaddend , we would expect our \DIFdelbegin \DIFdel{approach }\DIFdelend \DIFaddbegin \DIFadd{LDA model }\DIFaddend to perform comparatively poorly \DIFdelbegin \DIFdel{relative }\DIFdelend to BERT or 
other much larger \DIFaddbegin \DIFadd{general-purpose }\DIFaddend models. We suggest that bridging \DIFdelbegin \DIFdel{the tradeoff 
between }\DIFdelend \DIFaddbegin \DIFadd{this tradeoff 
between achieving }\DIFaddend high resolution within \DIFdelbegin \DIFdel{each content area versus }\DIFdelend \DIFaddbegin \DIFadd{a single content area and }\DIFaddend the ability to 
generalize to many \DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{diverse }\DIFaddend content areas will be an important challenge for 
future work\DIFdelbegin \DIFdel{in this domain. }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

\DIFaddbegin \DIFadd{At the opposite end of the spectrum from large language models, one could also
imagine using an even }\textit{\DIFadd{simpler}} \DIFadd{``model'' than LDA that relates the 
...
their contents at too }\textit{\DIFadd{broad}} \DIFadd{a semantic scale, the lack of specificity 
in this approach arises from considering too }\textit{\DIFadd{narrow}} \DIFadd{a semantic scale: 
...
\DIFadd{In other words, while the embedding spaces of more complex large language models 
...
resolution because they ``zoom }\textit{\DIFadd{in}}\DIFadd{'' too far. In this way, we view our 
...
\DIFaddend Another application for large language models that does \textit{not} require
explicitly modeling the content of individual lectures or questions is to
leverage \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend models' abilities to generate text. For example, generative text
...
needs in real time, and \DIFdelbegin \DIFdel{that are able to }\DIFdelend provide more nuanced feedback about
what learners know and what they do not know.

\DIFdelbegin \DIFdel{At the opposite end of the spectrum from large language models, one could also
imagine }\textit{\DIFdel{simplifying}} %DIFAUXCMD
\DIFdel{some aspects of our LDA-based approach by
...
relevant }\textit{\DIFdel{topics}} %DIFAUXCMD
\DIFdel{are discussed (e.g., when words like ``cloud,''
``atom,'' ``dense,'' etc., are mentioned }\textit{\DIFdel{together}}%DIFAUXCMD
\DIFdel{). The Jaccard
similarity-based matches, on the other hand, are strong when }\textit{\DIFdel{any}} %DIFAUXCMD
\DIFdel{of
these words are mentioned, even if they do not occur together.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We view our approach as occupying a sort of ``sweet spot,'' between much larger
...
%DIFDELCMD < %%%
\DIFdelend One limitation of our approach is that topic models contain no explicit
...
Within the past several years, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend global pandemic forced many educators to
...
(18 participants); Biology, AP Biology\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend or High school Biology (15
participants); Health and Medicine (1 participant); \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend other videos not listed
...
Forces} video \DIFdelbegin \DIFdel{, }\DIFdelend and 1 participant reported that they were not sure whether they
...
Computing (14 participants), College and careers (7 participants), \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend other
...
drawn \DIFaddbegin \DIFadd{from }\DIFaddend our team's prior coursework and areas of interest, along with internet
...
and might even come down to the ``strategy'' a given participant \DIFdelbegin \DIFdel{uses }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend to answer
...
confidence intervals we \DIFdelbegin \DIFdel{reported }\DIFdelend \DIFaddbegin \DIFadd{report }\DIFaddend for each correlation were estimated \DIFdelbegin \DIFdel{by
generating 10,000 }\DIFdelend \DIFaddbegin \DIFadd{from
}\DIFaddend bootstrap distributions of \DIFdelbegin \DIFdel{correlation coefficients }\DIFdelend \DIFaddbegin \DIFadd{10,000 correlation coefficients obtained }\DIFaddend by
...
Dirichlet Allocation; ][]{BleiEtal03} trained on a set of documents \DIFdelbegin \DIFdel{, }\DIFdelend to
...
(i.e., the union of all unique words \DIFdelbegin \DIFdel{, }\DIFdelend across all documents, excluding ``stop
words\DIFdelbegin \DIFdel{.}\DIFdelend ''). Conceptually, each topic is intended to give larger weights to words
...
describing how much \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend document (most probably) reflects each topic. To select
...
As illustrated in Figure~\ref{fig:sliding-windows}A, we \DIFdelbegin \DIFdel{start }\DIFdelend \DIFaddbegin \DIFadd{started }\DIFaddend by building up a
corpus of documents using overlapping sliding windows that \DIFdelbegin \DIFdel{span each video}\DIFdelend \DIFaddbegin \DIFadd{spanned each lecture}\DIFaddend 's
transcript. Khan Academy provides professionally created, manual transcriptions
of all \DIFaddbegin \DIFadd{lecture }\DIFaddend videos for closed captioning. However, such transcripts would not be
...
\texttt{youtube-transcript-api} Python package~\citep{Depo18}. \DIFdelbegin \DIFdel{The transcripts
}\DIFdelend \DIFaddbegin \DIFadd{Each transcript
}\DIFaddend consisted of one timestamped line of text for every few seconds (mean: 2.34~s;
standard deviation: 0.83~s) of spoken content in the \DIFdelbegin \DIFdel{video }\DIFdelend \DIFaddbegin \DIFadd{lecture }\DIFaddend (i.e., corresponding
...
length of (up to) $w = 30$ transcript lines \DIFdelbegin \DIFdel{, }\DIFdelend and assigned each window a
...
``thing,'' and ``two.'' This yielded sliding windows \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{containing }\DIFaddend an average of 73.8
remaining words, and \DIFdelbegin \DIFdel{lasting for }\DIFdelend \DIFaddbegin \DIFadd{spanning }\DIFaddend an average of 62.22~seconds. We treated the
text from each sliding window as a single ``document\DIFdelbegin \DIFdel{,}\DIFdelend '' and combined these
documents across the two \DIFdelbegin \DIFdel{videos}\DIFdelend \DIFaddbegin \DIFadd{lectures}\DIFaddend ' windows to create a single training corpus for
the topic model.

After fitting \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend topic model to the two \DIFdelbegin \DIFdel{videos}\DIFdelend \DIFaddbegin \DIFadd{lectures}\DIFaddend ' transcripts, we could use the
...
obtain topic vectors for each \DIFaddbegin \DIFadd{quiz }\DIFaddend question in our pool (see Supp.~Tab.~\questions).
Taken together, we obtained a \textit{trajectory} for each \DIFaddbegin \DIFadd{lecture }\DIFaddend video, describing
its path through topic space, and a single coordinate for each question
(Fig.~\ref{fig:sliding-windows}C). Embedding both \DIFdelbegin \DIFdel{videos }\DIFdelend \DIFaddbegin \DIFadd{lectures }\DIFaddend and all of the
questions using a common model enables us to compare the content from different
moments of \DIFdelbegin \DIFdel{videos}\DIFdelend \DIFaddbegin \DIFadd{the lectures}\DIFaddend , compare the content across \DIFdelbegin \DIFdel{videos}\DIFdelend \DIFaddbegin \DIFadd{lectures}\DIFaddend , and estimate potential
associations between specific questions and specific moments of \DIFdelbegin \DIFdel{video}\DIFdelend \DIFaddbegin \DIFadd{lecture content}\DIFaddend .
...
correlations between \DIFaddbegin \DIFadd{the topic vectors for }\DIFaddend any lecture timepoint and \DIFaddbegin \DIFadd{quiz }\DIFaddend question, taken over all
timepoints in the given lecture \DIFdelbegin \DIFdel{, and all five }\DIFdelend \DIFaddbegin \DIFadd{and all }\DIFaddend questions \textit{about} that
...
Here $t$ indexes the \DIFdelbegin \DIFdel{set }\DIFdelend \DIFaddbegin \DIFadd{time series }\DIFaddend of lecture topic vectors \DIFdelbegin \DIFdel{, }\DIFdelend $L$, and $i$ and $j$ index
the topic vectors of questions \DIFaddbegin \DIFadd{$Q$ }\DIFaddend used to estimate the \DIFdelbegin \DIFdel{knowledge trace, $Q$}\DIFdelend \DIFaddbegin \DIFadd{participant's knowledge}\DIFaddend . Note
that ``\DIFdelbegin \DIFdel{correct}\DIFdelend \DIFaddbegin \DIFadd{$\mathrm{correct}$}\DIFaddend '' denotes the set of indices of the questions the
participant answered correctly on the given quiz.

Intuitively, $\mathrm{ncorr}(x, y)$ is the correlation between two topic
vectors (e.g., the topic vector \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{$x$ for }\DIFaddend one timepoint in a lecture \DIFdelbegin \DIFdel{, $x$, }\DIFdelend and the
topic vector \DIFaddbegin \DIFadd{$y$ }\DIFaddend for one question \DIFdelbegin \DIFdel{, $y$}\DIFdelend \DIFaddbegin \DIFadd{on a quiz}\DIFaddend ), normalized by the minimum and
maximum correlations (across all timepoints $t$ and questions \DIFdelbegin \DIFdel{$Q$}\DIFdelend \DIFaddbegin \DIFadd{$j$}\DIFaddend ) to range
...
individual quiz questions could be used to reliably predict their \DIFdelbegin \DIFdel{ability }\DIFdelend \DIFaddbegin \DIFadd{abilities }\DIFaddend to
...
computed analogous knowledge estimates based on \DIFdelbegin \DIFdel{all other }\DIFdelend \DIFaddbegin \DIFadd{two different subsets of
}\DIFaddend questions the participant answered on the same quiz\DIFdelbegin \DIFdel{about }\DIFdelend \DIFaddbegin \DIFadd{: }\DIFaddend (1) \DIFaddbegin \DIFadd{all }\textit{\DIFadd{other}}
\DIFadd{questions about }\DIFaddend the same lecture as the target question (``Within-lecture'';
Fig.~\ref{fig:predictions}, middle rows), and (2) \DIFaddbegin \DIFadd{all questions about }\DIFaddend the other
of the two lectures (``Across-lecture''; Fig.~\ref{fig:predictions},
bottom rows).

In \DIFdelbegin \DIFdel{each }\DIFdelend \DIFaddbegin \DIFadd{performing these analyses, our null hypothesis is that the knowledge
estimates we compute based on the quiz questions' embedding coordinates do
}\textit{\DIFadd{not}} \DIFadd{provide useful information about participants' abilities to correctly answer
...
}\textit{\DIFadd{unweighted}} \DIFadd{proportion of correctly answered questions used to compute
...
knowledge for each target question using all }\textit{\DIFadd{other}} \DIFadd{questions (or all
}\textit{\DIFadd{other}} \DIFadd{questions about the same lecture) they answered on the same quiz.
...
single }\textit{\DIFadd{correctly}} \DIFadd{answered question as the target, the proportion of
remaining questions answered correctly would be $\frac{n - 1}{q - 1}$, whereas
if we hold out a single }\textit{\DIFadd{incorrectly}} \DIFadd{answered question, the proportion
...
}\textit{\DIFadd{lower}} \DIFadd{for target questions a participant answered correctly than for
those they answered incorrectly.
}

\DIFadd{To correct for this baseline difference under our null hypothesis, we used a
...
}\textit{\DIFadd{same}} \DIFadd{proportion of correctly answered questions. For each target
...
for the target question using all }\textit{\DIFadd{other}} \DIFadd{remaining questions. Since each
...
\DIFadd{In each }\DIFaddend version of this analysis (i.e., row in Fig.~\ref{fig:predictions}), and
...
questions \DIFaddbegin \DIFadd{(or all questions about a particular lecture) }\DIFaddend that participants
...
implicitly nested within levels of \texttt{lecture} and \DIFaddbegin \DIFadd{so }\DIFaddend do not require
explicit nesting in our model formula. We then iteratively removed random
effects from the maximal model until it successfully converged with a \DIFdelbegin \DIFdel{full rank (i.e., non-singular) 
}\DIFdelend \DIFaddbegin \DIFadd{full-rank
}\DIFaddend random effects variance-covariance matrix. \DIFaddbegin \DIFadd{We obtained the odds ratios reported
in Figure~\ref{fig:predictions} by exponentiating the estimated coefficient for
``}\texttt{\DIFadd{knowledge}}\DIFadd{'' from each fitted model. Conceptually, these odds ratios
...
GLMM's ability to \DIFdelbegin \DIFdel{discriminate between correctly and incorrectly answered
}\DIFdelend \DIFaddbegin \DIFadd{explain participants' success on individual quiz }\DIFaddend questions to
that of an analogous model \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{which assumed (as we assume under our null
hypothesis) that knowledge estimates for correctly and incorrectly answered
questions }\DIFaddend did \textit{not} \DIFdelbegin \DIFdel{consider estimated
knowledge}\DIFdelend \DIFaddbegin \DIFadd{systematically differ, on average}\DIFaddend . Specifically, we
used the same sets of observations \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend which we fit each ``full'' model to fit
a second ``null'' model \DIFdelbegin \DIFdel{, with the formula:
}\[
    \DIFdel{\mathtt{accuracy \sim (1\ \vert\ participant) + (1\ \vert\ question)}
}\]%DIFAUXCMD
\DIFdel{where ``}\texttt{\DIFdel{accuracy}}%DIFAUXCMD
\DIFdel{'', ``}\texttt{\DIFdel{participant}}%DIFAUXCMD
\DIFdel{'', and ``}\texttt{\DIFdel{question}}%DIFAUXCMD
\DIFdel{'' are as defined above. 
As with
our full models, the null models we fit for the “All questions” version of the analysis for each quiz contained an additional term, $\mathtt{(1\ \vert\ lecture)}$, where ``}\texttt{\DIFdel{lecture}}%DIFAUXCMD
\DIFdel{''is as defined above}\DIFdelend \DIFaddbegin \DIFadd{with the same random effects structure, but with
the coefficient for the fixed effect of ''}\texttt{\DIFadd{knowledge}}\DIFadd{'' constrained to zero
(i.e., we removed this term from the null model)}\DIFaddend . We then compared each full
model to its reduced (null) equivalent using a likelihood-ratio test (LRT).
Because the \DIFdelbegin \DIFdel{typical }\DIFdelend \DIFaddbegin \DIFadd{standard }\DIFaddend asymptotic $\chi^2_d$ approximation of the null
distribution for the LRT statistic ($\lambda_{LR}$) \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend anti-conservative for
\DIFdelbegin \DIFdel{models that differ in their random slope terms}\DIFdelend \DIFaddbegin \DIFadd{finite sample sizes}\DIFaddend ~\citep{GoldSimo00,ScheEtal08b,SnijBosk11}, we computed
$p$-values for these tests using a parametric bootstrap
procedure~\citep{DaviHink97,HaleHojs14}. For each of \DIFdelbegin \DIFdel{1}\DIFdelend \DIFaddbegin \DIFadd{10}\DIFaddend ,000 bootstraps, we used
...
$\lambda_{LR}$ statistics we may expect to observe \DIFdelbegin \DIFdel{under }\DIFdelend \DIFaddbegin \DIFadd{given data that conforms to
}\DIFaddend our null hypothesis. \DIFdelbegin \DIFdel{Following~\mbox{%DIFAUXCMD
\citet{Ewen03}}\hskip0pt%DIFAUXCMD
, we }\DIFdelend \DIFaddbegin \DIFadd{We }\DIFaddend computed a corrected $p$-value for our observed
$\lambda_{LR}$ as \DIFdelbegin \DIFdel{$\frac{r}{n}$}\DIFdelend \DIFaddbegin \DIFadd{$\frac{r+1}{n+1}$}\DIFaddend , where $r$ is the number of simulated model
comparisons that yielded a $\lambda_{LR}$ greater than \DIFdelbegin \DIFdel{or equal to }\DIFdelend our observed value and
$n$ is the number of simulations we ran (\DIFdelbegin \DIFdel{1}\DIFdelend \DIFaddbegin \DIFadd{10}\DIFaddend ,000).
...
reference question's \DIFdelbegin \DIFdel{embedding space }\DIFdelend \DIFaddbegin \DIFadd{embedding-space }\DIFaddend coordinate, where each successive sphere's
...
\DIFdelbegin \DIFdel{proportion correct }\DIFdelend \DIFaddbegin \DIFadd{proportion-correct }\DIFaddend scores across reference questions that were answered
...
of per-radius \DIFdelbegin \DIFdel{proportion correct }\DIFdelend \DIFaddbegin \DIFadd{proportion-correct }\DIFaddend scores intersected the 95\% confidence
...
model and participants' \DIFdelbegin \DIFdel{quiz }\DIFdelend performance on each \DIFaddbegin \DIFadd{quiz }\DIFaddend question, we can estimate
...
(e.g., to estimate topic vectors \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend word clouds, as in
...
value of an RBF centered on a question's coordinate $\mu$ \DIFdelbegin \DIFdel{, }\DIFdelend is given by:
\begin{equation}
...
\end{equation}
\DIFdelbegin \DIFdel{Intuitively, }\DIFdelend Equation~\ref{eqn:rbf-knowledge} computes the weighted proportion of correctly
